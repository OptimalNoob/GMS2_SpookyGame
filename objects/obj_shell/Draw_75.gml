if (isOpen) {
	var font_size = font_get_size(consoleFont);
	var help_text = "Escape - Close Console";
	
	draw_set_font(consoleFont);
	draw_set_valign(fa_bottom);
	draw_set_color(consoleColor);
	draw_set_alpha(consoleAlpha);
	draw_roundrect_ext(consolePaddingH, display_get_gui_height() - consolePaddingV,
					(consolePaddingH * 2) + (string_length(help_text) * font_size), display_get_gui_height() - (consolePaddingV * 2) - (font_size * 2),
					cornerRadius, cornerRadius, false);
	draw_set_alpha(1.0);
	draw_set_color(fontColor);
	draw_text(consolePaddingH + font_size, display_get_gui_height() - (consolePaddingV * 2), help_text);
